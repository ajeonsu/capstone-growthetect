...
export default function LoginPage() {
  ...
  return (
    ...
    >
      {/* ── CREDENTIALS STEP: Split Card ── */}
      {step === 'credentials' && (
        <div className="w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl flex flex-col sm:flex-row" style={{ minHeight: '480px' }}>
          {/* LEFT PANEL – logo with PixelCard animation */}
          <div className="flex items-center justify-center px-6 py-12 sm:w-[40%]"
            ...
            >
              ...
              />
              {/* Pixel canvas renders on top of logo via z-index in CSS */}
            </PixelCard>
          </div>

          {/* RIGHT PANEL – title + form */}
          <div className="flex flex-col items-center justify-center px-8 py-10 sm:w-[60%]"
            style={{ background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)' }}>

            {/* Title + motto above avatar */}
            <div className="text-center mb-7">
              <h1 className="font-extrabold text-white leading-tight whitespace-nowrap" style={{ fontSize: '2.4rem' }}>
                <ShinyText text="GROWTH" color="#86efac" shineColor="#ffffff" speed={3} />
                <span className="text-white">etect</span>
              </h1>
              <p className="text-xs text-gray-300 opacity-80 mt-1 whitespace-nowrap">
                Your Smart Partner in Student Growth Monitoring
              </p>
            </div>

            {/* Avatar */}
            <div className="user-avatar mb-2">
              <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-11 h-11 sm:w-14 sm:h-14 text-green-400">
            ...
            <form onSubmit={handleSubmit} className="w-full">
              ...
              <button type="submit" disabled={loading} className="btn-login w-full font-semibold py-3 sm:py-4 rounded-md text-base sm:text-lg">
      ...
      {/* ── 2FA STEP: kept as single column card ── */}
      {step === '2fa' && (
        <>
          {/* Title pinned to top on non-mobile */}
          <div className="hidden sm:block absolute top-8 left-0 right-0 text-center z-10">
            <h1 className="login-title font-extrabold text-white mb-1">
              <ShinyText text="GROWTH" color="#86efac" shineColor="#ffffff" speed={3} /><span className="text-white">etect</span>
            </h1>
            <p className="text-xs text-gray-300 opacity-80">Your Smart Partner in Student Growth Monitoring</p>
          </div>

          <div className="login-card w-full max-w-sm sm:max-w-md relative z-10 mx-auto">
            {/* Title on mobile */}
            ...
            <form onSubmit={handleVerify2FA}>
              <div className="user-avatar">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-9 h-9 sm:w-12 sm:h-12 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
              </div>
              ...
              <button type="submit" disabled={loading || twoFACode.length !== 6} className="btn-login w-full font-semibold py-3 sm:py-4 rounded-md text-base sm:text-lg">
